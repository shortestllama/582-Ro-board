...
class MyClass(GeneratedClass):
    ...
    def onInput_onStart(self):
        ...
        if random_integer >= 7 and random_integer < 10: # If the number is 7, 8, or 9 
            self.anim_speak.say("^start(you) You are terrible! ^wait(you)") # Talk trash with an animation
        elif  random_integer >= 10: # If the number is 10-12 
            self.anim_speak.say("^start(not know) Are you sure about that move? ^wait(not know)") # Talk trash in a different way
        else: # If the number is between 0 and 6
